...
- (void) updateWithView:(GLGView *)view {
    ...
    void (^randomizeColors)(void) = ^{
        red = [RangeProperty randomValueWithMinimum:0 maximum:1];
        blue = [RangeProperty randomValueWithMinimum:0 maximum:1];
        green = [RangeProperty randomValueWithMinimum:0 maximum:1];
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        CGFloat r = (CGFloat) arc4random();
        CGFloat point = fmodf(r, circumference);
        NSInteger height = floor([RangeProperty randomValueWithMinimum:0 maximum:5]);
